...
5. **专有名词处理**：人名、地名、机构名等专有名词保持英文原文，技术术语可根据需要翻译
6. **语气还原**：保持原文的情感和语气
...
### 专有名词与术语
- **专有名词**（人名、地名、机构名）：保持英文原文
  - 例如: "Kerry 去了 Swindon College"
- **技术术语**：可翻译或加注
  - 例如: "API（应用程序接口）"或直接 "API"
...
2. **读取所有 batch_*.txt 文件，全文一次性翻译**（保持上下文连贯）
3. 将完整翻译结果写入 translations.txt
4. 无需用户逐批确认

**⚠️ 翻译策略（最重要）：**
- **不要急于输出**：先完整理解所有批次内容
- **质量优先于速度**：宁可多花时间理解，也不要翻译后发现错位
- **全局视角**：把所有批次当作一个完整故事来翻译

**翻译前准备（必须执行）：**
1. **先读取所有 batch_*.txt 文件**，理解全文内容
2. 注意人物关系、称呼方式、语言风格
3. 识别重复出现的人名、地名、机构名（保持原文不翻译）
4. 理解对话的情感基调和幽默点
...
**专有名词处理规则：**
- **人名不翻译**：Kerry, Kurtan, Chi 等保持原文
- **地名不翻译**：Swindon, Brean Sands, Stroud 等保持原文
- **机构名不翻译**：Swindon College, Halfords 等保持原文
- **其他无实际含义的专有名词不翻译**
- 专有名词在中文句子中直接使用英文
- 示例：
  - ✅ "Kerry说她不想去Swindon。"
...
- 翻译输出的行数必须与输入行数完全相同
- **输入格式带行号前缀 `001→原文`，输出时保留行号 `001→译文`**

**输入格式示例：**
```
023→But how am I supposed to find someone out there
024→in this crazy goddamn messed up world?
```
...
**错误输出（绝对禁止）：**
```
023→可我怎样才能在这个疯狂的混乱世界里找到人呢？
（缺少024行，导致后续全部错位！）
```

行号前缀的好处：
- 翻译时可以清楚看到每行的全局编号
- finalize 脚本会验证行号连续性，缺行会报错
- 即使翻译看起来不完整或不通顺，也必须保持行数一致

**翻译完成后自查（写入 translations.txt 前必做）：**
1. ✅ 检查行数是否完全匹配（输入多少行，输出多少行）
2. ✅ 检查行号前缀是否连续（001, 002, 003...没有跳号）
3. ✅ 检查最后 5-10 行是否为空或明显错误
4. ✅ 检查场景描述【】是否对应英文 ALL CAPS
5. ✅ 检查疑问句是否有"？"或"吗"
6. ✅ 抽查 3-5 处，确认中英文内容实际匹配
7. ✅ 检查人名翻译是否全文一致
